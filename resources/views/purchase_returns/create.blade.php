...
@section('title', __('messages.purchaseRetern.return_products_to_supplier'))
    ...
    <div class="main_container main_menu_open">
        <div class="page_content">
            <h1 class="heading_title" style="margin-top: 90px; width:300px;">
                 @lang('messages.purchaseRetern.return_products_to_supplier')
            </h1>
        <!-- اختيار الفاتورة -->
        <form method="GET" action="{{ route('purchaseReturns.create') }}" class="mb-4">
            <div class="col-md-3">
                <label class="mb-2">@lang('messages.purchaseRetern.select_purchase_bill')</label>
                <select name="bill_id" class="form-control" onchange="this.form.submit()">
                    <option value="">@lang('messages.purchaseRetern.select_purchase_bill')</option>
                    @foreach ($bills as $b)
                        <option value="{{ $b->id }}" {{ (isset($bill) && $bill->id == $b->id) ? 'selected' : '' }}>
                            {{ __('messages.purchaseRetern.select_bill_option', ['id' => $b->id, 'name' => $b->supplier->name]) }}
                        </option>
        ...
        <form method="POST" action="{{ route('purchaseReturns.store') }}">
            ...
            <table class="table table-bordered">
                <thead class="thead-dark">
                    <tr>
                        <th>@lang('messages.purchaseRetern.product')</th>
                        <th>@lang('messages.purchaseRetern.original_quantity')</th>
                        <th>@lang('messages.purchaseRetern.returned_quantity')</th>
                        <th>@lang('messages.purchaseRetern.returned_amount') (₪)</th>
                        <th>@lang('messages.purchaseRetern.reason')</th>
                        <th>@lang('messages.purchaseRetern.cash_refund')</th>
                    </tr>
                </thead>
                <tbody>
            ...
            <button type="submit" class="btn btn-danger btn-block">@lang('messages.purchaseRetern.save_returns')</button>
        </form>
        @endif
    </div>
    </div>
